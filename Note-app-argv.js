...
if (process.argv[2].toString() === "list") {
  fs.readFile("./list.json", "utf8",  (erreur, data)=> {
    if (erreur) throw erreur; // we can generate error before we parse to JSON
...
// node .js add --tille newtiltle --body newbody
  ...
  fs.readFile("./list.json", "utf8",  (erreur, data) =>{
    ...
    fs.writeFile("./list.json", JSON.stringify(monJson),  (err) => {
...
// node Note-app.js remove -t title
  ...
  fs.readFile("./list.json", "utf8",  (erreur, data) => {
    ...
    fs.writeFile("./list.json", JSON.stringify(newlist),  (err)=> {
      if (err) return console.log(err);
    });
  });
}
